...
def filter_message(msg, mgr, all_actions=True):
    ...
    try:
        ...
        if ACTION_NONE is None: ACTION_NONE = gettext("Untouched").lower()
        if ACTION_COPY is None: ACTION_COPY = gettext("Copied").lower()
        if ACTION_MOVE is None: ACTION_MOVE = gettext("Moved").lower()
...
def filterer(mgr, config, progress):
    ...
    if not config.folder_ids:
        progress.error(gettext("You must specify at least one folder"))
        return
    ...
    dispositions = {}
    for f in mgr.message_store.GetFolderGenerator(config.folder_ids, config.include_sub):
        progress.set_status(gettext("Filtering folder '%s'") % (f.name))
    ...
    if "Error" in dispositions:
        err_text = gettext(" (%d errors)") % dispositions["Error"]
    dget = dispositions.get
    text = gettext("Found %d spam, %d unsure and %d good messages%s") % \
