...
def ReportMAPIError(manager, what, exc_val):
    ...
    if hr == mapi.MAPI_E_TABLE_TOO_BIG:
        err_msg = what + gettext(" failed as one of your\r\n" \
        ...
        err_msg = what + gettext(" failed due to an unexpected Outlook error.\r\n") \
                  + GetCOMExceptionString(exc_val) + "\r\n\r\n" + \
                  gettext("It is recommended you restart Outlook at the earliest opportunity\r\n\r\n" \
                    "This message will not be reported again until SpamBayes\r\n"\
                    "is restarted.")
...
class MAPIMsgStore:
    ...
    def __init__(self, outlook = None):
        ...
        self.session = mapi.MAPILogonEx(0, "", None, logonFlags)
...
class MAPIMsgStoreMsg:
    ...
    def MoveToReportingError(self, manager, folder):
        try:
            ...
            ReportMAPIError(manager, gettext("Moving a message"),
                            details.mapi_exception)
    def CopyToReportingError(self, manager, folder):
        try:
            self.MoveTo(folder)
        except MsgStoreException as details:
            ReportMAPIError(manager, gettext("Copying a message"),
                            details.mapi_exception)
